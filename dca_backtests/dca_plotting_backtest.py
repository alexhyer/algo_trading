...
"""
from dca_plotting_backtest_funcs import plot
...
for x in range(ma_1+derive_rate,length): #Main backtesting loop
    ...
    if last_state==1 and state==2: #0 to 1 longs open
        ...
        open_long_price.append(open_price_1)
    if last_state==1 and state==3: #0 to 2 longs open
        open_long_time.extend([time_min,time_min])
        open_long_price.extend([open_price_1,open_price_2])
    if last_state==1 and state==4: #0 to 3 longs open
        open_long_time.extend([time_min,time_min,time_min])
        open_long_price.extend([open_price_1,open_price_2,open_price_3])
        
    if last_state==2 and state==3: #1 to 2 longs open
        open_long_time.append(time_min)
        open_long_price.append(open_price_2)
    if last_state==2 and state==4: #1 to 3 longs open
        open_long_time.extend([time_min,time_min])
        open_long_price.extend([open_price_2,open_price_3])
        
    if last_state==3 and state==4: #2 to 3 longs open
        open_long_time.append(time_min)
        open_long_price.append(open_price_3)
    ...
    if last_state==5 and state==6: #0 to 1 shorts open
        open_short_time.append(time_min)
        open_short_price.append(open_price_1)
    if last_state==5 and state==7: #0 to 2 shorts open
        open_short_time.extend([time_min,time_min])
        open_short_price.extend([open_price_1,open_price_2])
    if last_state==5 and state==8: #0 to 3 shorts open
        open_short_time.extend([time_min,time_min,time_min])
        open_short_price.extend([open_price_1,open_price_2,open_price_3])
        
    if last_state==6 and state==7: #1 to 2 shorts open
        open_short_time.append(time_min)
        open_short_price.append(open_price_2)
    if last_state==6 and state==8: #1 to 3 shorts open
        open_short_time.extend([time_min,time_min])
        open_short_price.extend([open_price_2,open_price_3])
        
    if last_state==7 and state==8: #2 to 3 shorts open
        open_short_time.append(time_min)
        open_short_price.append(open_price_3)
